            ...
            with torch.no_grad():
                ...
                for _ in range(num_samples):
                    ...
                    for count, iteration in enumerate(range(int(steps))):
                        ...
                        # save
                        filename = f"step{count:06d}_batch{batch:03d}_step1_cov.png"
                        img.save(add + filename)'''

                        lam = sigma_r ** 2
                        b = H_adj(noisy_img) / (self.args.sigma_noise**2) + x_hat_1 / lam
                        z1 = torch.randn_like(x, device=self.device)  # for Σ_t term
                        z2 = torch.randn_like(noisy_img, device=self.device)  # for the data term
                        r = z1 / sigma_r + H_adj(z2) / self.args.sigma_noise
                        mu_t, _ = self.cg(b, x_hat_1, lam, max_iter=100, eps=1e-5, H=H, Ht=H_adj)
                        eps_t, _ = self.cg(r, x_hat_1*0, lam, max_iter=100, eps=1e-5, H=H, Ht=H_adj)
                        ...
                        # save
                        filename = f"step{count:06d}_batch{batch:03d}_step2_cov.png"
                        img.save(add + filename)'''

                        z0 = torch.randn_like(x, device=self.device)  # for the I term
                        
                        x = (t+delta) * x_star + (1 - t- delta) * z0
                        ...
                        # save
                        filename = f"step{count:06d}_batch{batch:03d}_step3_cov.png"
                        img.save(add + filename)'''
